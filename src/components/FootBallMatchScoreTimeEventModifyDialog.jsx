...
class FootBallMatchScoreTimeEventModifyDialog extends React.Component {
    ...
    render() {
        ...
        return <div className="qz-live-steps-div">
            ...
            <div
                className="qz-live-steps-content-large"
            >{getEventDetail()}</div>
            ...
            >
                <p className="mb-n"
                   style={{fontSize: 14}}
                >{currentEvent === START || currentEvent === SECOND_HALF ? "将删除" + eventType[currentEvent].text + "的所有数据，确定?" : "确定要删除吗?"}</p>
            </Modal>
        </div>;
    }
}
