...
import {delScoreboardById} from "../../../axios";
...
class ScoreBoard extends React.Component {
    ...
    onDelete = () => {
        this.props.scoreboard && delScoreboardById({id: this.props.scoreboard.id}).then((data) => {
    ...
    render() {
        ...
        const titleStyle = this.state.position && (this.state.position.title == null ? "" : {
            top: this.state.position.title.y * 120,
            left: this.state.position.title.x * 600
        })
        const hostnameStyle = this.state.position && (this.state.position.hostname == null ? "" : {
            top: this.state.position.hostname.y * 120,
            left: this.state.position.hostname.x * 600
        })
        const guestnameStyle = this.state.position && (this.state.position.guestname == null ? "" : {
            top: this.state.position.guestname.y * 120,
            left: this.state.position.guestname.x * 600
        })
        const hostscoreStyle = this.state.position && (this.state.position.hostscore == null ? "" : {
            top: this.state.position.hostscore.y * 120,
            left: this.state.position.hostscore.x * 600
        })
        const guestscoreStyle = this.state.position && (this.state.position.guestscore == null ? "" : {
            top: this.state.position.guestscore.y * 120,
            left: this.state.position.guestscore.x * 600
        })
        const hostshirtStyle = this.state.position && (this.state.position.hostshirt == null ? "" : {
            top: this.state.position.hostshirt.y * 120,
            left: this.state.position.hostshirt.x * 600
        })
        const guestshirtStyle = this.state.position && (this.state.position.guestshirt == null ? "" : {
            top: this.state.position.guestshirt.y * 120,
            left: this.state.position.guestshirt.x * 600
        })
        const timeStyle = this.state.position && (this.state.position.time == null ? "" : {
            top: this.state.position.time.y * 120,
            left: this.state.position.time.x * 600
        })
        return <div>
            ...
            <div className="qz-score-board-container dark-white">
                <img className="qz-score-board-background-img"
                     src={this.state.scoreBoard ? this.state.scoreBoard : this.props.scoreboard.scoreboardpic}/>
                ...
                <Draggable axis="both" bounds="parent" handle=".handle" {...dragHandlers}>
                    <span flag="hostname" style={hostnameStyle}
                          className="qz-score-board-team-name center handle">主队名</span>
                </Draggable>
                <Draggable axis="both" bounds="parent" handle=".handle" {...dragHandlers}>
                    <span flag="guestname" style={guestnameStyle}
                          className="qz-score-board-team-name center handle">客队名</span>
                </Draggable>
                <Draggable axis="both" bounds="parent" handle=".handle" {...dragHandlers}>
                    <span flag="hostscore" style={hostscoreStyle}
                          className="qz-score-board-score center handle">主分</span>
                </Draggable>
                <Draggable axis="both" bounds="parent" handle=".handle" {...dragHandlers}>
                    <span flag="guestscore" style={guestscoreStyle}
                          className="qz-score-board-score center handle">客分</span>
                </Draggable>
                <Draggable axis="both" bounds="parent" handle=".handle" {...dragHandlers}>
                ...
                <Draggable axis="both" bounds="parent" handle=".handle" {...dragHandlers}>
                    <div flag="hostshirt" style={hostshirtStyle} className="qz-score-board-shirt center handle">
                        ...
                        <img draggable={false}
                             src={this.state.hostShirt ? this.state.hostShirt : this.props.scoreboard.hostshirtpic}/>
                    </div>
                </Draggable>
                <Draggable axis="both" bounds="parent" handle=".handle" {...dragHandlers}>
                    <div flag="guestshirt" style={guestshirtStyle} className="qz-score-board-shirt center handle">
                        ...
                        <img draggable={false}
                             src={this.state.guestShirt ? this.state.guestShirt : this.props.scoreboard.guestshirtpic}/>
                    </div>
                </Draggable>
            </div>
            <div style={{width: 600}} className="center mt-s">
                <Button type="primary" onClick={() => {
                    this.props.callBackFunc({
                        id: this.props.scoreboard ? this.props.scoreboard.id : null,
